...
sizegen:
	go run ./go/tools/sizegen/sizegen.go \
		--in ./go/... \
		--gen github.com/vitess/vitess/go/pools/smartconnpool.Setting \
		--gen github.com/vitess/vitess/go/vt/schema.DDLStrategySetting \
		--gen github.com/vitess/vitess/go/vt/vtgate/engine.Plan \
		--gen github.com/vitess/vitess/go/vt/vttablet/tabletserver.TabletPlan \
		--gen github.com/vitess/vitess/go/sqltypes.Result
...
unit_test_cover: build dependency_check demo
	source build.env
	go test $(VT_GO_PARALLEL) -count=1 -failfast -covermode=atomic -coverpkg=github.com/vitess/vitess/go/... -coverprofile=coverage.out ./go/...
...
$(PROTO_GO_OUTS): minimaltools install_protoc-gen-go proto/*.proto
	$(VTROOT)/bin/protoc \
		...
		--go-vtproto_opt=pool=github.com/vitess/vitess/go/vt/proto/query.Row \
		--go-vtproto_opt=pool=github.com/vitess/vitess/go/vt/proto/binlogdata.VStreamRowsResponse \
		--go-vtproto_opt=pool=github.com/vitess/vitess/go/vt/proto/binlogdata.VStreamTablesResponse \
		-I${PWD}/dist/vt-protoc-21.3/include:proto $(PROTO_SRCS)
	cp -Rf github.com/vitess/vitess/go/vt/proto/* go/vt/proto
	rm -rf github.com/vitess/vitess/go/vt/proto/
