...
sizegen:
	go run ./go/tools/sizegen/sizegen.go \
		--in ./go/... \
		--gen mdibaiee/vitess/go/pools/smartconnpool.Setting \
		--gen mdibaiee/vitess/go/vt/schema.DDLStrategySetting \
		--gen mdibaiee/vitess/go/vt/vtgate/engine.Plan \
		--gen mdibaiee/vitess/go/vt/vttablet/tabletserver.TabletPlan \
		--gen mdibaiee/vitess/go/sqltypes.Result
...
unit_test_cover: build dependency_check demo
	source build.env
	go test $(VT_GO_PARALLEL) -count=1 -failfast -covermode=atomic -coverpkg=mdibaiee/vitess/go/... -coverprofile=coverage.out ./go/...
...
$(PROTO_GO_OUTS): minimaltools install_protoc-gen-go proto/*.proto
	$(VTROOT)/bin/protoc \
		...
		--go-vtproto_opt=pool=mdibaiee/vitess/go/vt/proto/query.Row \
		--go-vtproto_opt=pool=mdibaiee/vitess/go/vt/proto/binlogdata.VStreamRowsResponse \
		--go-vtproto_opt=pool=mdibaiee/vitess/go/vt/proto/binlogdata.VStreamTablesResponse \
		-I${PWD}/dist/vt-protoc-21.3/include:proto $(PROTO_SRCS)
	cp -Rf mdibaiee/vitess/go/vt/proto/* go/vt/proto
	rm -rf mdibaiee/vitess/go/vt/proto/
